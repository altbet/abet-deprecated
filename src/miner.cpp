            ...
            if (STAKING_ON_SEGWIT < pindexPrev->nTime) {
                ...
                for (size_t t = 1; t < pblock->vtx.size(); t++) {
                    ...
                    if (fDebug) {
                        ...
                        LogPrintf("CreateNewBlock : Accepted staking-on-segwit block found and was turned on 07/16/2019 @ 1:00am (UTC).\n");
					}
                    
                } else if (STAKING_ON_SEGWIT > pindexPrev->nTime) {
                    if (fDebug) {
                        LogPrintf("CreateNewBlock : Dang staking-on-segwit block found but current time is before 07/16/2019 @ 1:00am (UTC).\n");
                        return NULL;
					}
                }
            }
