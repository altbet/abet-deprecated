...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    /*if (block.IsProofOfStake()) {
        ...
        //Just to make sure the segwit staking block is accepted.
        LogPrintf("CheckBlock() : Kaboom Staking on segwit is working correctly.");
    }*/
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (STAKING_ON_SEGWIT < pindexPrev->nTime) {
        ...
        if (commitpos != -1) {
            ...
            LogPrintf("CheckBlock() : Boom staking-on-segwit was turned on 07/16/2019 @ 1:00am (UTC).\n");
        }
    } 
	...
	if (!fHaveWitness) {
		for (size_t i = 0; i < block.vtx.size(); i++) {
			if (!block.vtx[i].wit.IsNull()) {
				...
				LogPrintf("Before Full Segwit : Unexpected witness data found on this block prior to 07/16/2019 @ 1:00am (UTC).\n");
				fHaveWitness = true;
			}
		}
	}
